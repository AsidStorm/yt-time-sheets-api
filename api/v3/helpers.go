	...
	internalWrapper := func(callable handlerFunc, public bool) func(w http.ResponseWriter, r *http.Request) {
		return func(w http.ResponseWriter, r *http.Request) {
			...
			if !ctx.Session().IsAuthorized() && !public {
				...
				ctx.Logger().Infof("%s %s: %s", r.Method, r.RequestURI, errors.New("not authorized"))
				...
				if err != nil {
					ctx.Logger().Infof("%s %s: %s", r.Method, r.RequestURI, err)

					JsonError(w, status, err)
				} else {
					ctx.Logger().Infof("%s %s: done in %v", r.Method, r.RequestURI, elapsed)
